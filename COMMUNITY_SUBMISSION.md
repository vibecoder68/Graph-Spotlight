...
1. Unzip `outputs/graph-spotlight-0.1.5-local-install.zip`.
...
2. Create a GitHub release whose tag is exactly `0.1.5`.
...
Do not rely on `graph-spotlight-0.1.5-release-assets.zip` for the Obsidian validator. The zip is only a convenient way to move the files around; the GitHub Release itself must show `main.js`, `manifest.json`, and `styles.css` as separate attached assets.
